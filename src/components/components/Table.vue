<template lang="">
    <table class="table table-striped" >
        ...
        <tbody>
            <tr v-for="year, index in data">
                ...
                <td v-for="month, i in year.months" :key="`month-value-${i}`">
                    <div v-if="month != null" :class="setClassResults(0, month)">
                        {{ month.toFixed(2) }}€
                    </div>
                    <div v-else>
                        -
                ...
                <td :class="setBgColor(0, rowTotal(year.months))">
                   {{ rowTotal(year.months)}} 
                </td>
                <td>
                   <div v-if="type===0">
                       <router-link class="btn btn-sm btn-main">
      ...
      <ConfirmationModal v-if="isModalVisible" @close="closeModal" @handleConfirmDelete="confirmDelete" :title="'Sei sicuro di voler cancellare questo anno?'" :text="'Sei sicuro Una volta cancellato questo anno, verranno eliminate anche le relative entrate ed uscite e non sarà più possibile recuperarle. Vuoi procedere?'"  :type="type" />
</template>
<script setup>
